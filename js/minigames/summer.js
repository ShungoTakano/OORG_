    ...
    endGame(success) {
        ...
        overlay.innerHTML = `
            <div class="result-text ${success ? 'success' : 'failure'}">
                ${success ? '正解！野良犬が尻尾を持っていった！' : '失敗...真相にたどり着けなかった'}
            </div>
            ${success ? '<div style="margin: 10px; color: #aaa;">木の下から尻尾を回収した！</div>' : ''}
            <button class="menu-btn" id="summer-continue">続ける</button>
        `;
        container.appendChild(overlay);

        document.getElementById('summer-continue').addEventListener('click', () => {
